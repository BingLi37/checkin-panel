...
async def _check_in_via_oauth_relogin(account, account_index, provider_config, account_name):
	"""通过 OAuth 重新登录触发签到（agentrouter 等 provider）。

	agentrouter 的签到只在登录时触发，没有签到接口。
	需要 cloakbrowser 持久化 profile 已预登录 linux.do。
	"""
	print(f'[INFO] {account_name}: Using OAuth relogin check-in (no sign_in_path)')

	try:
		from panel.oauth_checkin import oauth_checkin
	except ImportError:
		print(f'[FAILED] {account_name}: panel.oauth_checkin not available')
		return False, None, None

	result = await oauth_checkin(
	...
	before_quota = result.get('before_quota')
	after_quota = result.get('after_quota')
	error = result.get('error')
	new_session = result.get('new_session_cookie')

	if not success:
		print(f'[FAILED] {account_name}: OAuth relogin failed - {error}')
		return False, {'quota': before_quota} if before_quota is not None else None, None

	if checked_in:
		print(f'[SUCCESS] {account_name}: Check-in successful (checked_in=True)')
	else:
		print(f'[INFO] {account_name}: Login successful but checked_in=False (already checked in today)')

	# If we got a new session cookie, store it back on the account config
	# so subsequent operations use the fresh session.
	if new_session:
		account.cookies = {'session': new_session}
		if is_debug_enabled():
			print(f'[INFO] {account_name}: Updated session cookie from OAuth login')

	info_before = {'quota': before_quota} if before_quota is not None else None
	info_after = {'quota': after_quota} if after_quota is not None else None
...
async def check_in_account(account: AccountConfig, account_index: int, app_config: AppConfig):
	...
	# agentrouter 等无签到接口的 provider：签到只在重新登录时触发
	if provider_config.needs_oauth_relogin():
		return await _check_in_via_oauth_relogin(
			account, account_index, provider_config, account_name
		)
	...
	try:
		...
		with httpx.Client(**client_kwargs) as client:
			...
			if user_info_after and user_info_after.get('success'):
				print(f'[INFO] {account_name}: User info retrieved (no sign_in_path — use OAuth relogin for check-in)')
